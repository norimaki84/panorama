...
var camera   = new THREE.PerspectiveCamera(40, width / height, 0.5, 1000); // カメラ作成。画角40、距離1〜1000の部分を表示できる。
camera.position.x = 0;
camera.position.y = 0;
camera.position.z = 0;
camera.lookAt(new THREE.Vector3(1, 0, 0));

creatArrow = function(angle){
    ...
    group.position = new THREE.Vector3(r*Math.cos(angle/180*Math.PI), -0.2, r*Math.sin(angle/180*Math.PI));
    group.rotation.set(0, (225-angle)/180*Math.PI, 0);
